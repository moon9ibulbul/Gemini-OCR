...
if [ ! -f "$WRAPPER_JAR" ]; then
    ...
    if [ -z "$distributionUrl" ]; then
        die "ERROR: Unable to locate distributionUrl in $PROPERTIES_FILE"
    fi

    distributionUrl=$( printf '%s' "$distributionUrl" | tr -d '\r' | sed 's@\\@@g' )
    gradleVersion=$( printf '%s' "$distributionUrl" | sed -n 's@.*/gradle-\([0-9][0-9.]*\)-.*@\1@p' )
    ...
    if command -v curl >/dev/null 2>&1; then
        if ! curl --fail --location --output "$WRAPPER_JAR" "$jarUrl"; then
            die "ERROR: Failed to download Gradle wrapper JAR"
        fi
    elif command -v wget >/dev/null 2>&1; then
        if ! wget --output-document="$WRAPPER_JAR" "$jarUrl"; then
            die "ERROR: Failed to download Gradle wrapper JAR"
        fi
    else
