...
internal object ImagePreprocessor {
    ...
    fun normalizeMeanVariance(mat: Mat): Mat {
        ...
        val result = Mat(mat.size(), CvType.CV32FC3)
        val floatMat = Mat()
        mat.convertTo(floatMat, CvType.CV32FC3)
        val meanScalar = Mat(1, 1, CvType.CV32FC3)
        meanScalar.put(0, 0, mean.map { it * 255.0 }.toDoubleArray())
        val stdScalar = Mat(1, 1, CvType.CV32FC3)
        stdScalar.put(0, 0, std.map { it * 255.0 }.toDoubleArray())
        Imgproc.subtract(floatMat, meanScalar, result)
